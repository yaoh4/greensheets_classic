...
/**
 * Custom class for handling things related to locating directories/files in the 
 * filesystem. It can be used, for example, for determining the physical filesystem 
 * location in which to look for a properties file, while avoiding the unpleasant 
 * answer of placing it in the same directory where the classes are or elsewhere on 
 * the classpath - even if the application is run from a JAR file. <br> 
 * Note that potential complications with multiple classloaders weren't given any
 * thought :).
