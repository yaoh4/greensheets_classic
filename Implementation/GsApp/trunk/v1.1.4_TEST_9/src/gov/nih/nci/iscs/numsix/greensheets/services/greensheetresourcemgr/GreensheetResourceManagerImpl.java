...
public class GreensheetResourceManagerImpl implements GreensheetResourceManager {
    ...
    private static final Logger logger = Logger.getLogger(GreensheetResourceManagerImpl.class);
    ...
    public GreensheetTemplateWrapper loadVelocityTemplate(String id) throws Exception {
        ...
        if (o != null) {
            ...
            if (this.frozen) {
                sqlSelect = "SELECT ft.id,ft.template_html FROM form_templates_t ft where ft.id=" + id;
                ...
                sqlSelect =
                    "SELECT ft.id, fgm.activity_code,fgm.appl_type_code, ft.template_html FROM form_templates_t ft, form_grant_matrix_t fgm where ft.id=fgm.ftm_id and ft.id="
                        + id;
                ...
                while (rs.next()) {
                    ...
                    String cache =
                        (
                            (Properties) AppConfigProperties.getInstance().getProperty(
                                GreensheetsKeys.KEY_CONFIG_PROPERTIES)).getProperty(
                            "greensheet.template.cache");
                    ...
                    while (st.hasMoreTokens()) {
                        ...
                        if (val.equalsIgnoreCase(type + actCode)) {
                            map.put(id, tw);
                            logger.debug("<<<<<<<<<<<<<<<<<<adding templId to Map " + id);
                        }
                    }
                }
