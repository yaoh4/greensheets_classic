...
public class GreensheetsUserServicesImpl implements GreensheetsUserServices {
	...
	private static final Logger logger = Logger.getLogger(GreensheetsUserServicesImpl.class);
	...
	public GsUser getUserByUserName(String userName) {
		...
		logger.debug(" The OracleId is " + nciPerson.getOracleId());
		...
		if (nciPerson.getOracleId() != null) {
			...
			try {
				isI2eAccountValid = nciPersonDAO.isI2eAccountValid(nciPerson.getOracleId());

			} catch (Exception e) {
				e.printStackTrace();
			}

			if (!isI2eAccountValid)
				return null;

			gsUser = userRoleDataDAO.getUserRolesActivitiesNPEIDs(nciPerson.getOracleId());
		...
		if (gsUser != null) {
			...
			gsUser.setUserPreferences(this.greensheetsUserPreferencesServices.readUserPrefernces(gsUser.getUserName(),
	...
	public static void main(String args[]) {
		...
		logger.debug("nciOracleID of the user is " + gsUser.getNciPerson().getOracleId());
	}
}
 ...
 * SPRING_LDAP_JARs=%SPRING_LDAP%\dist\modules\spring-ldap-core-1.3.0.RELEASE.
 * jar
 * 
 * set SPRING_CORE=C:\dev\tools\spring-framework-2.5.6.SEC01 set
 * SPRING_CORE_JARs=%SPRING_CORE%\dist\spring.jar set
 * MISC_JARs=%SPRING_CORE%\lib\jakarta-commons\commons-lang.jar;%SPRING_CORE%\
 * lib\jakarta-commons\commons-logging.jar;%SPRING_CORE%\lib\jakarta-commons\
 * commons-beanutils.jar;%SPRING_CORE%\lib\jakarta-commons\commons-collections.
 * jar
 * 
 ...
 * lib\jakarta-commons\commons-logging.jar;%SPRING_CORE%\lib\jakarta-commons\
 * commons-beanutils.jar;%SPRING_CORE%\lib\jakarta-commons\commons-collections.
 * jar;%SPRING_CORE%\lib\jakarta-commons\commons-pool.jar
 * 
