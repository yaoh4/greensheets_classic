...
public class GreensheetUserMgrImpl implements GreensheetUserMgr {
     ...
     */
    public GsUser findUserByUserName(String userName) throws GreensheetBaseException {
    ...
    private void getUserDBData(GsUser user) throws GreensheetBaseException {
        ...
        try {
            ...
            cs = conn.prepareCall("{call notifications_pkg.get_role_param_list(?,?,?,?,?)}");
            ...
            cs.registerOutParameter(4, java.sql.Types.VARCHAR); //cancer activites
            cs.registerOutParameter(5, java.sql.Types.VARCHAR); //role_usage codes. Not Used
            ...
            cs = conn.prepareCall("{call notifications_pkg.get_user_pd_ca_details(?,?,?,?,?,?)}");
            cs.setString(1, user.getOracleId());

            // Set the OUT parameters to register
            cs.registerOutParameter(2, java.sql.Types.VARCHAR); //p_pd_flag. Not Used
            cs.registerOutParameter(3, java.sql.Types.VARCHAR); //p_ca_flog. Not Used
            cs.registerOutParameter(4, java.sql.Types.VARCHAR); //P_npe_ids.
            cs.registerOutParameter(5, java.sql.Types.VARCHAR); //p_ru_codes. Not Used
            cs.registerOutParameter(6, java.sql.Types.VARCHAR); //p_ca_codes codes. Not Used
