...
public class SearchProgramGrantsAction extends GsBaseDispatchAction {
	...
	/** restorePreferences */
	public ActionForward restorePreferences(ActionMapping mapping, ActionForm form, HttpServletRequest req,
		...
		// pgmPrefForm);
		this.restoreUserPreferences(GreensheetActionHelper.getGreensheetUserSession(req), pgmPrefForm);
		// enhancement for cancel action
		this.readUserGrants(req, pgmPrefForm, true);
	...
	public ActionForward search(ActionMapping mapping, ActionForm form, HttpServletRequest req, HttpServletResponse res)
		...
		// set program preferences form values
		this.setProgramPreferencesForm(GreensheetActionHelper.getGreensheetUserSession(req), pgmPrefForm); // This
																											// invokes
																											// readUserPreferences()
																											// if
																											// required.
		...
		this.readUserGrants(req, pgmPrefForm, false);
	...
	/** cancel */
	public ActionForward cancel(ActionMapping mapping, ActionForm form, HttpServletRequest req, HttpServletResponse res)
			throws Exception {
		ProgramPreferencesForm pgmPrefForm = (ProgramPreferencesForm) form;
		this.restoreUserPreferences(GreensheetActionHelper.getGreensheetUserSession(req), pgmPrefForm);
		this.readUserGrants(req, pgmPrefForm, false);
	...
	private void setGrantsSourcesLOV(HttpServletRequest req) throws Exception {
		...
		grantSources.add(new LabelValueBean("My Portfolio", Constants.PREFERENCES_MYPORTFOLIO));
		...
		grantSources.add(new LabelValueBean("My Cancer Activity" + userCancerActivities,
				Constants.PREFERENCES_MYCANCERACTIVITY));
		grantSources.add(new LabelValueBean("All NCI Grants", Constants.PREFERENCES_ALLNCIGRANTS));
	...
	private void setGrantTypesLOV(HttpServletRequest req) {
		...
		grantTypes.add(new LabelValueBean("Competing Grants", Constants.PREFERENCES_COMPETINGGRANTS));
		grantTypes.add(new LabelValueBean("Non-Competing Grants", Constants.PREFERENCES_NONCOMPETINGGRANTS));
	...
	private void setProgramPreferencesForm(GreensheetUserSession gus, ProgramPreferencesForm prefs) throws Exception {// Abdul
																														// Latheef:
																														// Changed
																														// the
																														// return
	...
	private void setUserSessionPaylineOption(ProgramPreferencesForm prefs, GreensheetUserSession gus) {
		if (prefs.getOnlyGrantsWithinPayline() == null) {
			gus.setPaylineOnly(false);
		} else {
			if (prefs.getOnlyGrantsWithinPayline().equals(Constants.PREFERENCES_YES)) {
	...
	/** setUserSessionPaylineOption */
	private void setUserSessionMyPortfolioOption(ProgramPreferencesForm prefs, GreensheetUserSession gus) {
	...
	private void readUserPreferences(GreensheetUserSession gus, ProgramPreferencesForm prefsForm) throws Exception {
		...
		Map<String, String> userPreferences = gus.getUser().getUserPreferences();

		prefsForm.grantSource = userPreferences.get(Constants.PREFERENCES_GRANT_SOURCE_KEY);
		prefsForm.grantType = userPreferences.get(Constants.PREFERENCES_GRANT_TYPE_KEY);
		prefsForm.mechanism = userPreferences.get(Constants.PREFERENCES_GRANT_MECHANISM_KEY);
		prefsForm.onlyGrantsWithinPayline = userPreferences.get(Constants.PREFERENCES_GRANT_PAYLINE_KEY);
	...
	private void restoreUserPreferences(GreensheetUserSession gus, ProgramPreferencesForm prefsForm) throws Exception {
	...
	private void readUserGrants(HttpServletRequest req, ProgramPreferencesForm pgmPrefForm, boolean filterCancelled) throws Exception {
		
		GreensheetUserSession gus = GreensheetActionHelper.getGreensheetUserSession(req);
		GsUser gsUser = gus.getUser();
		List formGrants = greensheetsFormGrantsService.findGrants(gsUser, pgmPrefForm.getGrantSource(),
				pgmPrefForm.getGrantType(), pgmPrefForm.isPaylineOnly(), // includeOnlyGrantsWithinPayline
				pgmPrefForm.getMechanism(), pgmPrefForm.getGrantNumber(), pgmPrefForm.getLastName(),
				pgmPrefForm.getFirstName(), filterCancelled);

		List<FormGrantProxy> list = GreensheetActionHelper.getFormGrantProxyList(formGrants, gsUser);
		req.getSession().setAttribute("GRANT_LIST", list);
	...
	private void defaultUserGrants(HttpServletRequest req, ProgramPreferencesForm pgmPrefForm) throws Exception {

		GreensheetUserSession gus = GreensheetActionHelper.getGreensheetUserSession(req);
		GsUser gsUser = gus.getUser();
		/*
		 * List formGrants = greensheetsFormGrantsService.findGrants(gsUser,
		 * pgmPrefForm.getGrantSource(), pgmPrefForm.getGrantType(),
		 ...
		 */
		List formGrants = greensheetsFormGrantsService.findGrants(gsUser, false, true, true, true);
