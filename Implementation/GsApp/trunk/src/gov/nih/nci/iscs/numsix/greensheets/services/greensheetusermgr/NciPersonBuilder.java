...
import gov.nih.nci.iscs.numsix.greensheets.utils.LdapConnectionHelper;
...
/**
 * Creates a NciPerson object user the NCI LDAP.
 * 
 * @see NciPerson.
 * 
...
public class NciPersonBuilder {
	...
	private String[] PERSON_ATTRS_DETAILS = { UID, EMAIL, CN, FULL_NAME,
			MANAGER, FIRST_NAME, MIDDLE_INITIAL, LAST_NAME, TITLE,
			FUNCTIONAL_ROLE, GROUP_MEMBERSHIP, ORGANIZATION, EXTERNAL_ORG,
			ADMIN_UNIT, MEMBER, TFSID, ORACLE_ID };

	private static final Logger logger = Logger
			.getLogger(NciPersonBuilder.class);
	...
	public NciPerson getPersonByUserName(String name)
		...
		try {
			...
			Properties p = (Properties) AppConfigProperties.getInstance()
					.getProperty(GreensheetsKeys.KEY_LDAP_PROPERTIES);
			...
			Attributes attributes = context.getAttributes(person
					.getDistinguishedName(), PERSON_ATTRS_DETAILS);
	...
	private void populatePersonDetails(NciPerson person,
			Attributes personAttributes) throws GreensheetBaseException {
		try {
