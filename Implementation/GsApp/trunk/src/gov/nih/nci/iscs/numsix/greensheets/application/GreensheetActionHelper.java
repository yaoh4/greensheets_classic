...
public class GreensheetActionHelper {
    ...
    public static GreensheetUserSession getGreensheetUserSession(
        ...
        /**
         * FY to assume for retrieving grants list in DEV and TEST environments with old data - change-able via UI only by super-users and stored in
         * GreensheetUserSessions
         */
        String currentFYtoAssume = "";
        if (gus != null) {
            currentFYtoAssume = gus.getUser().getUserPreferences().get("FY_TO_ASSUME");
            if (currentFYtoAssume == null) {
                currentFYtoAssume = "";
            }
        }
        ...
        if (req.getSession().getAttribute(GreensheetsKeys.ACTUAL_USER_SESSION) == null) {
            req.getSession().setAttribute(GreensheetsKeys.ACTUAL_USER_SESSION,
        ...
        if (gus == null) {
            ...
            if (remoteUserName != null) {
                ...
                if (user == null) {
                    throw new GreensheetBaseException("You do not have permission to access this application.");
                }
            } else if (userId != null) {
     ...
     * @throws Exception
     */
    public static void setFormDraftDisplayInfo(HttpServletRequest req, String formUid)
            throws Exception {
        ...
        displaySubmit = false;
        displaySave = false;
        displayRmcLetterOption = false;
        displayGmsAndPdCode = false;
        displayPoc = false;
        disablePoc = false;
        disabled = false;
        req.setAttribute("grantDetailUrl", grantDetailUrl);
    ...
    public static List<FormGrantProxy> getFormDraftGrantProxyList(
            List formGrantsList, GsUser user) {
        //System.out.println("Inside  getFormDraftGrantProxyList formGrantsList size is " +formGrantsList.size());
        List<FormGrantProxy> grantProxyList = new ArrayList<FormGrantProxy>();
        ...
        //System.out.println("Inside  getFormDraftGrantProxyList grantProxyList size is " +grantProxyList.size());
        return grantProxyList;
    }
}
