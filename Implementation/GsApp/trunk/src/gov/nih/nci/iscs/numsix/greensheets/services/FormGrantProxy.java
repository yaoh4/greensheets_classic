...
public class FormGrantProxy extends FormGrant implements Serializable {
    ...
    private String getReadyForPGSubmissionIcon() {

        String thumbUpIcon = "";

        if (this.getPgmGsReadyFlag() != null) {
            String status = this.getPgmFormStatus();
            if (!status.equalsIgnoreCase("SUBMITTED") && !status.equalsIgnoreCase("FROZEN")) {
                if (this.getPgmGsReadyFlag().equalsIgnoreCase("Y")) {
                    thumbUpIcon = "<img src=\"images/thumb_up.png\" width=\"15\" height=\"18\" border=\"0\" alt=\"PGM GS is ready for submission.\">";
                }
            }
        }
        return thumbUpIcon;
    ...
    /**
     * Returns the URL for opening the greensheet and sets the ordering sequence of the greensheet which is NotStarted IN Control = A Saved IN Control
     * = B UnSubmitted = C Saved NOT IN Control =D Submitted = E NOT Started Not In Control = F Frozen = G
     * 
     * @return String
     */
        ...
        if (Status != null) {
            if (Status.equalsIgnoreCase("SUBMITTED")) {
                ...
                if (this.isOnControl()
                    ...
                    attname = "B" + this.getLatestBudgetStartDate();
                } else {
                ...
                if (this.isOnControl()
                            && (user.getRole().equals(GsUserRole.PGM_ANL) || user.getRole()
                                    .equals(GsUserRole.PGM_DIR))) {
                    attname = "A" + this.getLatestBudgetStartDate();
                } else {
